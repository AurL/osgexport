...
class BlenderAnimationToAnimation(object):
    ...
    def createAnimation(self, target=None):
        ...
        if self.config.bake_animations or self.needBake(self.object):
            self.action = osgbake.bakeAnimation(self.config.scene, self.config.bake_frame_step, self.object, has_action=self.has_action, use_quaternions=self.config.use_quaternions)
